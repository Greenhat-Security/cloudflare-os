...
describe("subscribeToActions", () => {
  ...
  it("replays a hook toggled after the cutoff, carrying the toggled state", async () => {
    ...
    let client = await openFakeOverseer({
      ...
      // The client delegates the flip to the durable hook-transition machinery (persisted intent,
      // generation fence, provider compensation), which needs far more of a workspace than this
      // fixture forges. What this test is about is the action log, so run the production stamping
      // helper directly and leave the machinery to overseer-hooks.test.ts.
      impl: {
        disableHook: async (id: number) => {
          if (id !== hook.id) throw new Error("Invalid hook ID.");
          hook.enabled = false;
          overseerTestInternals.stampBindHookAction(storage, hook.actionId, false);
        },
      },
    });
