...
describe("restarting sessions when verification scope widens", () => {
  ...
  it("...but a legacy connection still blocks a build collaborator",
    ...
    await expect(impl.listObserverRequirements("build")).rejects.toThrow(/reconnected/);
  }));
  ...
  it("...and still blocks a use collaborator once a gadget binds it",
    ...
    impl.bindWorkpiece(100, "DB", 1);
    await expect(impl.listObserverRequirements("use")).rejects.toThrow(/reconnected/);
  }));
...
describe("hooks widen use scope", () => {
  ...
  // Upstream covered two in-flight hook races here by driving the client's inline enable/disable
  // against a storage-only fake. This fork replaced that flip with a durable transition (persisted
  // intent, generation fence, provider compensation), which a fake of that shape cannot run, so the
  // same properties are covered against the real machinery in overseer-hooks.test.ts: "makes delete
  // win over an in-flight enable and removes the durable tombstone last" and "does not resurrect a
  // hook deleted while its disable was in flight".
