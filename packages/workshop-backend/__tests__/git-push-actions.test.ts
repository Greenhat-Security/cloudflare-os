...
async function inOverseer(name: string, fn: (impl: any) => Promise<void>): Promise<void> {
  ...
  await runInDurableObject(stub, async (instance: OverseerDurableObject) => {
    ...
    // submitAction re-reads deployment policy for the connection it stages against, so the
    // connection has to exist as a record rather than only as an id.
    impl.storage.gatekeepers.put({
      id: GATEKEEPER,
      resourceTitle: `Connection ${GATEKEEPER}`,
      class: {} as any,
      creationSpec: {
        type: "gatekeeper",
        vendorId: "testvendor",
        resourceUrl: "https://example.com/push",
...
describe("push authorization through the Overseer chokepoints", () => {
  ...
  it("cleans a queued push's marks when its gatekeeper is removed", async () => {
    await inOverseer("push-gatekeeper-removed", async impl => {
      ...
      // A connection with approvals still staged cannot be removed: they are settleable only
      // through its facet, so removing it would strand them. Denying the push is what releases its
      // marks; the cleanup loop in removeGatekeeper stays as a backstop for records that predate
      // that rule.
      expect(() => impl.removeGatekeeper(GATEKEEPER)).toThrow(/pending approval/);

      impl.getGatekeeperFacet = () => ({ rejectAction: async () => {} }) as any;
      await impl.rejectPendingAction(record.id, USER);
      expect(marksOf(impl, record.id)).toStrictEqual([]);
