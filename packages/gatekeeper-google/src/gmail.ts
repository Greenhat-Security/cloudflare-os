...
import {isReadOnly, READ_ONLY_MESSAGE, readOnlyTypes, requireWritable} from "./read-only";
...
type GmailContext = {
  api: GmailApi;
  /** Green Hat fork: deployment-wide read-only policy; see read-only.ts. */
  readOnly: boolean;
...
async function submitAction(
  ...
  // Every Gmail write is queued through here, so one guard covers send, reply, forward, the
  // thread and message mutations, and the label actions.
...
export class GmailGatekeeperImpl extends DurableObject<Env, GmailGatekeeperImplProps>
  ...
  async getTypeScriptTypes(): Promise<string> { return readOnlyTypes(this.env, TYPES_CODE); }
  ...
  async applyAction(actionId: number, _cache: RpcStub<GitCache>): Promise<void> {
    // An approval staged before the switch must not land after it.
